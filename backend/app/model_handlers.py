...
def initialize_client(provider):
    ...
    try:
        if provider == "Cerebras (llama3.3-70b)":
            raise NotImplementedError("Cerebras no está disponible en este entorno.")
        elif provider == "Groq (qwen-qwq-32b)":
            return _init_groq()
        elif provider == "DeepSeek (DeepSeek-R1)":
            return _init_deepseek()
        elif provider == "Azure OpenAI (o3-mini)":
            client = AzureOpenAI(
        ...
        elif provider == "Azure OpenAI (o4-mini)":
            client = AzureOpenAI(
                api_key=os.getenv("O4MINI_API_KEY"),
                azure_endpoint=os.getenv("O4MINI_ENDPOINT"),
                api_version="2025-01-01-preview"
            )
            return client, "o4-minirag", os.getenv("O4MINI_ENDPOINT")
        elif provider == "Google (Gemini-2.0-Flash)":
            return _init_google(GOOGLE_MODEL_2_0)
        elif provider == "Google (Gemini-2.5-Pro)":
            return _init_google(GOOGLE_MODEL_2_5)
        elif provider == "GPT-4o (New)":
            client = AzureOpenAI(
                api_key=GPT4O_NEW_API_KEY,
                azure_endpoint=GPT4O_NEW_ENDPOINT,
                api_version="2025-01-01-preview"
            )
            return client, "gpt-4o", GPT4O_NEW_ENDPOINT
        elif provider == "Bedrock (Claude 3.7 Sonnet)":
            return _init_bedrock("BEDROCK_CLAUDE_MODEL_ID", "BEDROCK_CLAUDE_MODEL_ID")
        elif provider == "Bedrock (Nova Pro)":
            return _init_bedrock("BEDROCK_NOVA_PRO_MODEL_ID", "BEDROCK_NOVA_PRO_MODEL_ID")
        elif provider == "Azure OpenAI (Phi 4)":
            base_url = f"{PHI4_AZURE_ENDPOINT}models/chat/completions?api-version=2024-05-01-preview"
            return None, PHI4_MODEL, base_url
        elif provider == "GPT-4.1":
            client = AzureOpenAI(
                api_key=os.getenv("GPT41_API_KEY"),
                azure_endpoint=os.getenv("GPT41_ENDPOINT"),
                api_version="2025-01-01-preview"
            )
            return client, "gpt-4.1", os.getenv("GPT41_ENDPOINT")
        elif provider == "Azure OpenAI (gpt-4.1-mini)":
            client = AzureOpenAI(
                api_key=os.getenv("GPT41MINI_API_KEY"),
                azure_endpoint=os.getenv("GPT41MINI_ENDPOINT"),
                api_version=os.getenv("GPT41MINI_API_VERSION", "2025-01-01-preview")
            )
            model = os.getenv("GPT41MINI_MODEL_NAME", "gpt-4.1-mini")
            return client, model, os.getenv("GPT41MINI_ENDPOINT")
        else:
...
def _invoke_google(base_url, system_prompt, user_prompt, max_tokens):
    ...
    data = {
        ...
        "generationConfig": {"temperature": 0.01, "maxOutputTokens": max_tokens}
    }
...
def _invoke_bedrock_nova(client, model, system_prompt, user_prompt):
    ...
    request_body = {
        "messages": messages,
        "inferenceConfig": {"max_new_tokens": 4096, "temperature": 0.01, "top_p": 0.99, "top_k": 20}
    }
...
def invoke_model(
    client, model, base_url, system_prompt, user_prompt, provider,
    max_tokens=1500, stream=True, synthesis_stream=False
):
    ...
    try:
        if provider in ["Google (Gemini-2.0-Flash)", "Google (Gemini-2.5-Pro)"]:
            ...
            return _invoke_bedrock_claude(client, model, system_prompt, user_prompt, max_tokens)
        elif provider == "Bedrock (Nova Pro)":
            return _invoke_bedrock_nova(client, model, system_prompt, user_prompt)
        elif provider == "Azure OpenAI (Phi 4)":
            return _invoke_phi4(base_url, model, system_prompt, user_prompt, max_tokens)
        else:
            params = {
                "model": model,
                "messages": [
            ...
            if "Azure OpenAI" in provider or "GPT-4o" in provider or "DeepSeek" in provider:
            ...
            if synthesis_stream and params["stream"]:
                ...
                print("[invoke_model] Retornando respuesta completa (modo stream, pero no synthesis_stream)")
