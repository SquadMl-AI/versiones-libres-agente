    ...
    with (
        patch(
            ...
            return_value=MagicMock(
                __enter__=MagicMock(return_value=MagicMock(read=lambda: '{"id": "test"}', write=MagicMock()))
            ),
        ),
        patch("json.load", return_value={"id": "test"}),
        patch("json.dump") as mock_dump,
    ):
        load_module_from_source("services/indexing_service.py", "src_services_indexing_service")
