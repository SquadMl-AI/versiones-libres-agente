...
def setup_mock_indexing():
    ...
    return load_module_from_source('utils/indexing_pipeline.py', 'src_indexing_pipe_full'), mock_search, mock_fitz_doc, mock_blob, mock_aoai
...
def test_document_processing_indexing_orchestrator():
    ...
    with patch.object(pipeline, 'reading_processing_documents', return_value=([{"content": "c", "docnm_kwd": "d", "docnm": "d", "bloque": "b", "kb_id": "1", "page_number": 1}], [])):
        with patch.object(pipeline, 'semantic_chunking', return_value=[{"content": "chunk", "docnm_kwd": "d"}]):
            with patch.object(pipeline, 'normalize_data', return_value=[{"content": "chunk"}]):
                with patch.object(pipeline, 'create_knowledge_base', return_value=[{"e": 1}]):
                    res = pipeline.document_processing_indexing_orchestrator({}, "index")
                    assert len(res) == 1
    ...
    with patch("builtins.open", MagicMock()):
        with patch("json.load", return_value={}):
            with patch("json.dump", MagicMock()):
                with patch.object(module.DocumentProcessingPipeline, 'document_processing_indexing_orchestrator') as mock_orch:
                    ...
                    kb = module.pipeline.document_processing_indexing_orchestrator({})
