...
def prepare_module():
    ...
    sys.modules['bs4'].BeautifulSoup = lambda text, parser: type('Soup', (), {'get_text': lambda self, separator=' ': text.replace('<p>', '').replace('</p>', '')})()
    ...
    sys.modules['utils.ai_services'].AzureServices = type('AzureServices', (), {'AzureBlobStorage': type('B', (), {}), 'DocumentIntelligence': type('D', (), {}), 'AzureOpenAI': type('O', (), {'__init__': lambda self: setattr(self, 'client_embeddings', object())}), 'AzureIASearch': type('S', (), {})})
...
def test_semantic_chunking_returns_chunks_with_metadata():
    ...
    json_data = [{'docnm_kwd': 'doc', 'docnm': 'doc.pdf', 'bloque': 'b1', 'kb_id': 'kb1', 'content': 'Primera parte. Segunda parte.', 'page_number': 1}]
